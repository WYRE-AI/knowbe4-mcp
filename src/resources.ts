/**
 * MCP Resource Handlers for the KnowBe4 MCP Server.
 *
 * Exposes the MCP Apps (SEP-1865) user-card UI via ListResources and
 * ReadResource handlers. The card HTML is embedded at build time
 * (src/generated/user-card-html.ts) so it serves identically from stdio and
 * Node HTTP without any filesystem access at runtime.
 */
...
export function listResources(): McpResource[] {
  return [
    {
      uri: USER_CARD_RESOURCE_URI,
      name: "KnowBe4 User Card",
      description: "Interactive MCP Apps card rendering a KnowBe4 user's security-awareness profile",
...
export function readResource(uri: string): McpResourceContent {
  if (uri === USER_CARD_RESOURCE_URI) {
    return {
      ...
      text: applyBrandInjection(USER_CARD_HTML, resolveBrandFromEnv()),
    };
  }
